...
%% Thresholding based on Distance and skin colour
...
%      wrist_coordinates_depth = skeleton_joints_depth(11,:)
    ...
    if ~isempty(hand_coordinates) && ~isempty(wrist_coordinates) && hand_coordinates(1)~=0 && hand_coordinates(1)<640 && hand_coordinates(2) < 480 
      ...
      SEPERATE_BACKGROUND2 =0 ;
      
       if SEPERATE_BACKGROUND2
           [row,col]  = find(img_depth==0);
    ...
    %Detect Skin
%             [r,c,~] = find(Cb>=50 & Cb<=127 & Cr>=133 & Cr<=173);
%           [r,c,v] = find(Cb>=77 & Cb<=127 & Cr>=133 & Cr<=173);
          [r,c,v] = find(Cb>=110 & Cb<=135 & Cr>=133 & Cr<=150);
          
% [r,c,v] = find(hue_channel < .09897);
